...
# iFQI_linear_closed_form
echo "launch train ifqi linear closed form"
# --gres=gpu:1 -p gpu
submission_train_ifqi_linear_closed_form_1=$(sbatch -J $EXPERIMENT_NAME --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=500M --time=1:00:00 --output=/dev/null launch_job/car_on_hill/train_ifqi_linear_closed_form.sh -e $EXPERIMENT_NAME -ns $N_PARALLEL_SEEDS)