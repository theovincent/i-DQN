...
do
    ...
    submission_train_ifqi=$(sbatch -J C_train_ifqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=3:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_ifqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_ifqi_%a.out -p amd,amd2 launch_job/car_on_hill/train_ifqi.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope)
    ...
    submission_evaluate_ifqi=$(sbatch -J C_evaluate_ifqi --dependency=afterok:$submission_id_train_ifqi,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=150Mc --time=10:00 --output=out/car_on_hill/$EXPERIMENT_NAME/evaluate_ifqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/evaluate_ifqi_%a.out -p amd,amd2 launch_job/car_on_hill/evaluate_ifqi.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope)
done