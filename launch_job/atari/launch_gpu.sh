...
do
    ...
    submission_train_idqn_1=$(sbatch -J t_$EXPERIMENT_NAME --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=$(( 2 * $N_PARALLEL_SEEDS )) --mem-per-cpu=17Gc --time=3-00:00:00 --output=/dev/null --gres=gpu:1 -p amd,amd2,rtx,rtx2 launch_job/atari/train_idqn.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope -g -ns $N_PARALLEL_SEEDS)
    ...
    submission_evaluate_idqn_1=$(sbatch -J e_$EXPERIMENT_NAME --dependency=after:$submission_id_train_idqn_1 --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=$(( 1 * $N_PARALLEL_SEEDS )) --mem-per-cpu=6Gc --time=3-00:00:00 --output=/dev/null -p amd,amd2 launch_job/atari/evaluate_idqn.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope -ns $N_PARALLEL_SEEDS)
    ...
    submission_evaluate_idqn_2=$(sbatch -J e_$EXPERIMENT_NAME --dependency=after:$submission_id_train_idqn_2 --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=$(( 1 * $N_PARALLEL_SEEDS )) --mem-per-cpu=6Gc --time=3-00:00:00 --output=/dev/null -p amd,amd2 launch_job/atari/evaluate_idqn.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope -r -ns $N_PARALLEL_SEEDS)
done