...
do
    ...
    submission_train_idqn_1=$(sbatch -J L_train_idqn_$EXPERIMENT_NAME --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=1Gc --time=15:30:00 --output=out/atari/$EXPERIMENT_NAME/$bellman_iterations_scope\_train_idqn_%a.out -p amd,amd2 launch_job/atari/train_idqn.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope)
    ...
    submission_train_idqn_2=$(sbatch -J L_train_idqn_$EXPERIMENT_NAME --dependency=afterok:$submission_id_train_idqn_1 --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=1Gc --time=15:30:00 --output=out/atari/$EXPERIMENT_NAME/$bellman_iterations_scope\_train_idqn_%a.out -p amd,amd2 launch_job/atari/train_idqn.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope -r)
