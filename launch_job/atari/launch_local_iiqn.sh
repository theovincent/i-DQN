...
if ! tmux has-session -t hackatari; then
    tmux new-session -d -s hackatari
    echo "Created new tmux session - hackatari"
fi
...
then
    ...
    tmux send-keys -t hackatari "$seed_command" ENTER "$train_command" ENTER
fi