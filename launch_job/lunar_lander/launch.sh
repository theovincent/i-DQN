...
do
    ...
    submission_train_idqn=$(sbatch -J L_train_idqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=1Gc --time=15:30:00 --output=out/lunar_lander/$EXPERIMENT_NAME/$bellman_iterations_scope\_train_idqn_%a.out -p amd,amd2 launch_job/lunar_lander/train_idqn.sh -e $EXPERIMENT_NAME -b $bellman_iterations_scope)
