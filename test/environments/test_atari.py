...
class TestAtariEnv(unittest.TestCase):
    ...
    def test_reset(self) -> None:
        env = AtariEnv(self.key, self.name, self.gamma)

        state_0 = env.reset()
        state_1 = env.reset()

        self.assertGreater(np.linalg.norm(state_1 - state_0), 0, f"random seed {self.random_seed}")
    ...
    def test_step_deterministic_stochasticity(self) -> None:
        ...
        env_bis = AtariEnv(self.key, self.name, self.gamma)
    ...
    def test_step_frame_stacking(self) -> None:
        ...
        self.assertEqual(env.env.unwrapped.ale.lives(), env.n_lives - 1, f"random seed {self.random_seed}")

    def test_store_load(self) -> None:
        env_to_store = AtariEnv(self.key, self.name, self.gamma)
        action_key = self.key
        env_to_store.reset()
